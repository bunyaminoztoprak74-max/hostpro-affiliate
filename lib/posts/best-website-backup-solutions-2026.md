...
A Windows system-backup tool may complement a self-managed Windows VPS or a local administration machine, but it is not a substitute for a WordPress-aware backup workflow. The destination must still be independent from the device being protected. Do not count a second partition on the same physical disk as an off-site copy.
